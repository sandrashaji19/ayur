...
app.get("/", (req, res) => {
  ...
  app.post("/signup",pdfupload,(req,res)=>{
    ...
    connection.query('insert into Account(name,password,email,phone) value (?,?,?,?)',
    [name,password,email,phone],
    (error,results,feilds)=>{
      if(error){
        res.render(__dirname+"/registration",{
          sql:'error in insertion'
        })
        throw error;
      }else{
        console.log(results)
...
const senderEmail = 's620212024@gmail.com';
  ...
  app.post('/booktreatment',pdfupload,(req,res)=>{
    ...
    if(req.session.userid!=undefined){
        connection.query('insert into treatmentBooking (uid,email,phone,treatment) value(?,?,?,?)',
        [req.session.userid,req.body.email,req.body.phone,req.body.treatment],(error,results,feilds)=>{
            if(error){
                res.send(error)
            }else{
                console.log(results)
  ...
  app.get('/bookingProducts',(req,res)=>{
    console.log()
  })
  ...
  app.post("/addbooking",(req,res)=>{
    connection.query('INSERT into booking (pid,pname,pimage,userid) VALUES(?,?,?,?)',[req.session.pid,req.session.pname,req.session.pimage,req.session.userid],(error,results,fields)=>{
      if(error){
        console.error(error);
      }
      else{
